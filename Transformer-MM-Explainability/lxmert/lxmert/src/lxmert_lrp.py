...
class LxmertAttention(nn.Module):
    ...
    def forward(self, hidden_states, context, attention_mask=None, output_attentions=False, save_values=False):
...
# This class is added by Goro Kobayashi
...
class LxmertCrossAttentionLayer(nn.Module):
    ...
    def save_att_norm(self, att_norm):
        self.att_norm = att_norm.data

    def get_att_norm(self):
        return self.att_norm

    def forward(self, input_tensor, ctx_tensor, ctx_att_mask=None, output_attentions=False, save_values=False):
        inp1, inp2 = self.clone(input_tensor, 2)
        # print(save_values)
        output = self.att(inp1, ctx_tensor, ctx_att_mask, output_attentions=output_attentions, save_values=save_values)
        if save_values:
            # print(len(output))
            norms_outputs = self.norm(None, output[1], output[2], self.output.dense)
            self.save_att_norm(norms_outputs[-1])
